...
include("../../../../../../arquivos/config.php");
// Criando sessões
// Definindo o nome e inicializando sessão
$eva['config'] = (isset($config))? $config : array();
session_name(md5(  (substr($eva['config']['url'],-1) == '/')? $eva['config']['url'] : $eva['config']['url'].'/' ));
session_start();
// Seguranca - Evitar acesso direto a pagina
if(!isset($_SESSION['diretorio_arquivos'])) exit;
$_SESSION['diretorio_arquivos'] = (!empty($_SESSION['diretorio_arquivos']))? $_SESSION['diretorio_arquivos'] : '';

$extensao_arquivos = (!empty($_GET['extensao_arquivos']))? explode(',',$_GET['extensao_arquivos']) : array();
$extensao_arquivos = (!isset($_GET['extensao_arquivos']) && isset($_SESSION['extensao_arquivos']))? $_SESSION['extensao_arquivos'] : $extensao_arquivos;
$_SESSION['extensao_arquivos'] = (count($extensao_arquivos) > 0)? $extensao_arquivos : array('html', 'doc', 'ppt', 'pps', 'rtf', 'xls', 'txt', 'gif', 'jpeg', 'jpg', 'png', 'pdf', 'zip', 'rar', 'tgz', 'gz', 'pdf', 'xml','swf','avi','mpg','mpeg','exe','mp3','mp4');

$config['url'] = (substr($config['url'],-1) != "/")? $config['url']."/" : $config['url'];
$config['idioma_padrao'] = ($config['idioma_padrao'] == 'br')? 'pt_br' : $config['idioma_padrao'];
$config['tamanho_max_upload_arquivos'] = intval($config['tamanho_limite_arquivo'])>0 ? ($config['tamanho_limite_arquivo']) : 10; //em Mb -- Adicionado por Rander em 01/05/2008
...
*/
$MY_DOCUMENT_ROOT     = "../../../../../../".$config['download'].$_SESSION['diretorio_arquivos']; //* system path to the directory you want to manage the files and folders

/* MY_BASE_URL  Not used in htmlarea3-plugin version  */
$MY_BASE_URL          = $config['url'].$config['download'].$_SESSION['diretorio_arquivos'];
...
*/
$MY_URL_TO_OPEN_FILE  = $config['url'].$config['download'].$_SESSION['diretorio_arquivos'];
...
$MY_LIST_EXTENSIONS  = $_SESSION['extensao_arquivos'];
/*
 MY_ALLOW_EXTENSIONS
 MY_DENY_EXTENSIONS
...
*/
$MY_ALLOW_EXTENSIONS = $_SESSION['extensao_arquivos'];
...
*/
$MY_MAX_FILE_SIZE   = $config['tamanho_max_upload_arquivos']*1024*1024;
