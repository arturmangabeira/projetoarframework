...
"use strict";	
	...
	$.fn.fmatter.showlink = function(cellval, opts) {
        ...
        $.fn.fmatter.linkModal = function(cellval, opts) {
		var op = {baseLinkUrl: opts.baseLinkUrl,modal: opts.modal,showAction:opts.showAction, addParam: opts.addParam || "", target: opts.target, idName: opts.idName},
		target = "", idUrl;
		if(opts.colModel !== undefined && !$.fmatter.isUndefined(opts.colModel.formatoptions)) {
			op = $.extend({},op,opts.colModel.formatoptions);
		}
		if(op.target) {target = 'target=' + op.target;}
                ...
                if(op.modal == 'true'){
                    idUrl = "javascript:gerarModal('"+op.baseLinkUrl +'&'+ op.idName+'='+opts.rowId+"')";
                }else{
                    idUrl = op.baseLinkUrl +'&'+ op.idName+'='+opts.rowId;
                }
		if($.fmatter.isString(cellval) || $.fmatter.isNumber(cellval)) {	//add this one even if its blank string
			return "<a "+target+" href=\"" + idUrl + "\">" + cellval + "</a>";
		}
